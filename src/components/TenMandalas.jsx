  ...
  return createPortal(
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-3xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden">
        {/* Header */}
        <div className="bg-gradient-to-r from-orange-600 to-red-600 text-beige p-6 relative">
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 hover:bg-beige/20 rounded-full transition"
          >
            <X size={24} />
          </button>
            ...
            <div className="space-y-3">
              {mandala.famousHymns && mandala.famousHymns.length > 0 ? (
                mandala.famousHymns.map((hymn, index) => (
                  <div
                    key={index}
                    className="bg-cream rounded-xl p-4 border-l-4 border-orange-500 shadow-sm hover:shadow-md transition"
                  >
        ...
        <div className="border-t border-orange-200 bg-beige/80 p-6">
          <button
            ...
            className="w-full bg-gradient-to-r from-orange-500 to-red-500 text-beige py-4 rounded-xl font-semibold flex items-center justify-center gap-3 hover:from-orange-600 hover:to-red-600 transition shadow-lg hover:shadow-xl"
          >
            <Headphones size={24} />
